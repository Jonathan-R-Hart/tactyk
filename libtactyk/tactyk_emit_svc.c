#include "tactyk_asmvm.h"
#include "tactyk_emit.h"
#include "tactyk_emit_svc.h"

void tactyk_emit_svc__configure(struct tactyk_emit__Context *emit_context, char *base_tokens_src) {

}

void tactyk_emit_svc__init(struct tactyk_asmvm__Context *asmvm_ctx) {

}

void tactyk_emit_svc__define_text(struct tactyk_asmvm__Context *asmvm_ctx) {

}

void tactyk_emit_svc__label(struct tactyk_asmvm__Context *asmvm_ctx) {

}

void tactyk_emit_svc__mem_external(struct tactyk_asmvm__Context *asmvm_ctx) {

}

void tactyk_emit_svc__mem_empty(struct tactyk_asmvm__Context *asmvm_ctx) {

}
void tactyk_emit_svc__mem_data(struct tactyk_asmvm__Context *asmvm_ctx) {

}
void tactyk_emit_svc__mem_text(struct tactyk_asmvm__Context *asmvm_ctx) {

}

void tactyk_emit_svc__begin_cmd(struct tactyk_asmvm__Context *asmvm_ctx) {

}
void tactyk_emit_svc__register(struct tactyk_asmvm__Context *asmvm_ctx) {

}
void tactyk_emit_svc__word(struct tactyk_asmvm__Context *asmvm_ctx) {

}
void tactyk_emit_svc__text(struct tactyk_asmvm__Context *asmvm_ctx) {

}
void tactyk_emit_svc__int(struct tactyk_asmvm__Context *asmvm_ctx) {

}
void tactyk_emit_svc__float(struct tactyk_asmvm__Context *asmvm_ctx) {

}
void tactyk_emit_svc__comparator(struct tactyk_asmvm__Context *asmvm_ctx) {

}
void tactyk_emit_svc__end_cmd(struct tactyk_asmvm__Context *asmvm_ctx) {

}
